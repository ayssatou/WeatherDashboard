...
function applyData(city, data, forecast) {
    ...
    $("#data-date").text(date.getUTCMonth() + "/" + date.getUTCDate() + "/" + date.getUTCFullYear());
    ...
    $("#cur-icon").attr("src",`https://openweathermap.org/img/wn/${data.current.weather[0].icon}@2x.png`);

    for(var i = 0; i < 5; i++){
        date = new Date(parseInt(forecast.list[i*8].dt + "000"));
        $("#forecast-date-" + (i+1)).text(date.getUTCMonth() + "/" + date.getUTCDate() + "/" + date.getUTCFullYear());
        ...
        $("#forecast-icon-" + (i+1)).attr("src",`https://openweathermap.org/img/wn/${forecast.list[i*8].weather[0].icon}@2x.png`);
    }
}
